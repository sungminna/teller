...
from backend.shared.models.fact_check import FactCheckResult
...
class TestFactCheckerAgent:
    ...
    async def test_source_verification(self, fact_checker_agent):
        ...
        assert len(result.sources) > 0
        assert any("NASA" in source or "scientific" in source.lower() for source in result.sources)
    ...
    async def test_fact_checking_accuracy_benchmark(self, fact_checker_agent):
        ...
        accuracy = (
            correct_predictions / high_confidence_predictions
        ...
        ), f"High confidence rate {confidence_rate:.2%} below 80% threshold"
    ...
    async def test_multilingual_fact_checking(self, fact_checker_agent):
        ...
        for claim_data in multilingual_claims:
            result = await fact_checker_agent.verify_claim(
                claim_data["claim"], language=claim_data["language"]
            )
        ...
        ), f"Multilingual accuracy {multilingual_accuracy:.2%} below 85% threshold"
...
class TestFactCheckResult:
    ...
    def test_fact_check_result_validation(self):
        ...
        with pytest.raises(ValueError):
            FactCheckResult(
                claim="", verdict=True, confidence=0.9, sources=[], reasoning=""  # Empty claim
            )
...
class TestFactCheckingPerformance:
    ...
    async def test_accuracy_under_load(self, fact_checker_agent):
        ...
        assert (
            high_confidence_count / len(results) >= 0.80
        ), "Too few high-confidence predictions under load"
