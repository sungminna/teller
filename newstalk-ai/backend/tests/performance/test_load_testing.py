...
class TestSystemPerformance:
    ...
    async def test_api_response_times(self, test_client, performance_test_config):
        ...
        for endpoint, metrics in response_times.items():
            assert metrics['p95'] <= target_response_time, \
                f"{endpoint} P95 response time {metrics['p95']:.2f}s exceeds target {target_response_time}s"
            assert metrics['average'] <= target_response_time * 0.7, \
    ...
    async def test_concurrent_user_load(self, test_client, performance_test_config):
        ...
        async def simulate_user_session():
            ...
            try:
                ...
                content_response = await test_client.get("/api/v1/news/personalized", headers=headers)
        ...
        assert total_time <= test_duration * 2, \
    ...
    async def test_voice_generation_performance(self, test_client):
        ...
        for content in test_contents:
            ...
            with patch('backend.services.voice_service.generate_audio') as mock_voice:
                ...
                response = await test_client.post("/api/v1/voice/generate", json={
                    "text": content,
        ...
        for metric in performance_metrics:
            ...
            assert metric["processing_time"] <= expected_time + 2.0, \
    ...
    async def test_database_performance(self, test_db_session):
        ...
        operations = [
            ...
            ("INSERT", "INSERT INTO user_interactions (user_id, article_id, interaction_type) VALUES (1, 1, 'view')"),
...
class NewsApiUser(HttpUser):
    ...
    def track_analytics(self):
        ...
        self.client.post("/api/v1/analytics/track", headers=self.headers, json={
            "event": "content_viewed",
            "article_id": "test_article",
            "duration": 60
        })
    ...
    def generate_voice(self):
        ...
        self.client.post("/api/v1/voice/generate", headers=self.headers, json={
...
class TestLoadTesting:
    ...
    def test_api_load_testing(self):
        ...
        # Verify performance requirements
        assert stats.total.avg_response_time <= 2000, \
            f"Average response time {stats.total.avg_response_time}ms exceeds 2000ms"
        
        assert stats.total.failure_count / stats.total.num_requests <= 0.05, \
    ...
    def test_stress_testing(self):
        ...
        for user_count in user_counts:
            ...
            results[user_count] = {
                ...
                'error_rate': stats.total.failure_count / stats.total.num_requests if stats.total.num_requests > 0 else 0,
