...
from backend.shared.models.pipeline_result import PipelineResult
...
class TestAirflowLangGraphIntegration:
    ...
    async def test_full_pipeline_execution(self, dag_bag, sample_dag_run_data, test_db_session):
        ...
        for task in dag.topological_sort():
            ...
            try:
                ...
                task_execution_results[task.task_id] = {
                    'state': State.SUCCESS,
                    'result': result
                }
        ...
        assert all(
            ...
            for result in task_execution_results.values()
    ...
    @pytest.mark.asyncio
    async def test_content_analysis_task(self, dag_bag, sample_dag_run_data, content_analyzer_agent):
        ...
        with patch('airflow.models.TaskInstance.xcom_pull') as mock_xcom:
            ...
            with patch('backend.services.voice_service.generate_audio') as mock_voice:
                ...
                assert all(
                    ...
                    for item in result['voice_content']
                )
    ...
    async def test_quality_validation_task(self, dag_bag, sample_dag_run_data, quality_thresholds):
        ...
        with patch('airflow.models.TaskInstance.xcom_pull') as mock_xcom:
            ...
            assert result['quality_report']['overall_score'] >= quality_thresholds['pipeline_success_rate']
            assert result['quality_report']['fact_checking_accuracy'] >= quality_thresholds['fact_checking_accuracy']
            assert result['quality_report']['voice_quality_score'] >= quality_thresholds['voice_quality_score']
