...
from .exceptions import (
    NewsTeamError, ErrorCategory, ErrorSeverity, 
...
class AsyncProcessorPool:
    ...
    def __init__(self, config: AsyncConfig = None):
        ...
        logger.info(f"AsyncProcessorPool initialized with max_concurrency={self.config.max_concurrency}")
    ...
    async def process_items(
        ...
        try:
            ...
            if self.stats.processed_items > 0:
                self.stats.average_time_per_item = self.stats.processing_time / self.stats.processed_items
    ...
    async def _process_batch(
        ...
        for i in range(0, len(items), self.config.batch_size):
            ...
            logger.debug(f"Batch {i//self.config.batch_size + 1} completed: {successful}/{len(batch)} successful")
            ...
            try:
                ...
                if self._current_memory_mb > memory_threshold:
                    if not self._backpressure_active:
                        ...
                        logger.warning(f"Backpressure activated: {self._current_memory_mb:.1f}MB > {memory_threshold:.1f}MB")
                else:
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(_run_async_in_thread, func, args, kwargs, timeout, retries)
...
async def _run_with_timeout_and_retry(func, args, kwargs, timeout, retries):
    ...
    retry_manager = RetryManager(AsyncConfig(
