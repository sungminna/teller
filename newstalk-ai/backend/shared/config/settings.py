...
class KafkaSettings:
    ...
    # 개별 토픽 이름 (하위 호환성)
    raw_news_topic: str = "news.raw"
    processed_news_topic: str = "news.processed"
    user_feedback_topic: str = "user.feedback"
    realtime_updates_topic: str = "realtime.updates"
...
class APISettings:
    ...
    pipeline_max_processing_time: int = 300
...
class NewsTalkSettings(BaseSettings):
        ...
        try:
            ...
            if self.environment == Environment.PRODUCTION:
                required_settings = [
                    ...
                    (
                        self.api.secret_key != "your-secret-key-change-this",
    ...
    def _is_valid_host(self, host: str) -> bool:
        ...
        return host == "localhost" or re.match(ipv4_pattern, host) or re.match(domain_pattern, host)
    ...
    def _setup_logging(self):
        ...
        # 기본 로거 설정
        logging.basicConfig(
            level=log_level, format=self.monitoring.log_format, filename=self.monitoring.log_file
        )
    ...
    def _get_kafka_bootstrap_servers(self) -> List[str]:
        ...
        if is_k8s:
            ...
            logger.info("🔧 Kubernetes 환경 감지 - kafka:9092 사용")
            return ["kafka:9092"]
        elif is_docker:
            # Docker Compose 환경: 내부 네트워크 사용
            try:
                # kafka 호스트 해결 가능한지 확인
                socket.gethostbyname("kafka")
                logger.info("🐳 Docker 환경 감지 - kafka:29092 사용")
                return ["kafka:29092"]
            except socket.gaierror:
                # kafka 호스트 해결 불가능하면 localhost 사용
                logger.warning("⚠️ kafka 호스트를 해결할 수 없습니다. localhost:9092를 사용합니다.")
                return ["localhost:9092"]
        else:
            # 로컬 개발 환경
            logger.info("🖥️ 로컬 개발 환경 - localhost:9092 사용")
...
def yaml_config_settings_source(settings: BaseSettings) -> Dict[str, Any]:
    ...
    config_files = [f"config.{env}.yaml", f"config.{env}.yml", "config.yaml", "config.yml"]
        ...
        try:
            ...
            if self.settings.redis.host:
                self._redis_client = redis.from_url(
                    self.settings.get_redis_url(), encoding="utf-8", decode_responses=True
                )
        ...
        try:
            ...
            if persist and self._redis_client:
                await self._redis_client.set(
                    f"config:{key}", json.dumps(value, default=str), ex=3600  # 1시간 TTL
                )
