...
class AlertRequest(BaseModel):
    message: str
    severity: str = Field(
        default="info", description="Alert severity: info, warning, error, critical"
    )
    category: str = Field(
        default="system",
        description="Alert category: system, performance, cost, business, security",
    )
...
async def get_business_metrics():
    ...
    try:
        ...
        return {"status": "success", "metrics": metrics, "timestamp": datetime.utcnow().isoformat()}
...
async def get_cost_analytics(days: int = 7):
    ...
    try:
        ...
        return {"status": "success", "analytics": analytics, "period_days": days}
...
async def get_performance_insights(time_range_hours: int = 24):
    ...
    try:
        ...
        return {"status": "success", "insights": insights, "time_range_hours": time_range_hours}
...
async def track_llm_call_manually(
    ...
    try:
        result = await langfuse_manager.track_llm_call(
            model=model, prompt=prompt, response=response, usage=usage, metadata=metadata
        )
...
async def update_thresholds(thresholds: Dict[str, float]):
    ...
    try:
        ...
        if invalid_thresholds:
            raise HTTPException(
                status_code=400, detail=f"Invalid threshold names: {list(invalid_thresholds)}"
            )
...
async def get_monitoring_config():
    ...
    try:
        ...
        config = {
            "langfuse": {
                "enabled": hasattr(settings, "LANGFUSE_PUBLIC_KEY")
                and settings.LANGFUSE_PUBLIC_KEY is not None,
                "host": getattr(settings, "LANGFUSE_HOST", None),
            },
            "slack": {
                "enabled": hasattr(settings, "SLACK_WEBHOOK_URL")
                and settings.SLACK_WEBHOOK_URL is not None,
