...
class SSEConnectionManager:
    ...
    async def send_to_all(self, event_type: str, data: Dict[str, Any]):
        ...
        await cache_manager.cache_set(
            f"sse_broadcast:{datetime.utcnow().timestamp()}", message, expire=300  # 5 minutes
        )
...
@router.get("/events")
async def stream_events(request: Request, current_user: User = Depends(get_current_user)):
    """
...
async def send_notification(
    ...
    try:
        ...
        await publish_realtime_update(
            event_type, {**data, "sender_user_id": current_user.id, "target_user_id": user_id}
        )
...
async def get_streaming_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        kafka_status = {
            ...
            "processors": (
                len(stream_processor.processors) if hasattr(stream_processor, "processors") else 0
            ),
        }
...
@router.get("/metrics")
async def get_streaming_metrics(current_user: User = Depends(get_current_user)):
    """
        ...
        return {
            ...
            "time_series": {
                ...
                "avg_pipeline_duration": (
                    sum(s[1] for s in pipeline_samples) / len(pipeline_samples)
                    if pipeline_samples
                    else 0
                ),
