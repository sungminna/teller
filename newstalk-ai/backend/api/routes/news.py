...
from ..middleware.auth import TokenData, get_current_user, get_current_user_optional
...
async def ask_question(
    request: Request, qa_request: QARequest, current_user: TokenData = Depends(get_current_user)
):
    """
    대화형 Q&A 시스템
    뉴스 내용에 대한 심화 질문 처리
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            article = await conn.fetchrow(
                "SELECT * FROM news_articles WHERE id = $1", qa_request.article_id
            )
...
async def get_trending_news(
    request: Request, limit: int = 10, current_user: TokenData = Depends(get_current_user_optional)
):
    """
    최적화된 실시간 트렌딩 뉴스 조회
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            articles_need_scoring = [
                ...
                if article["score_updated_at"] is None
                or (datetime.utcnow() - article["score_updated_at"]).total_seconds() > 3600
            ]

            if articles_need_scoring:
                logger.info(
                    f"Scheduling trending score calculation for {len(articles_need_scoring)} articles"
                )
            ...
            trending_news.append(
                {
                    ...
                    "scoreUpdatedAt": (
                        article["score_updated_at"].isoformat()
                        if article["score_updated_at"]
                        else None
                    ),
                }
            )
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            for article in articles_need_scoring:
                interaction = next(
                    (i for i in interaction_data if i["article_id"] == article["id"]), None
                )
...
def calculate_trending_score(article, interaction):
    ...
    # 가중 점수 계산
    interaction_score = (
        view_count * 1.0 + bookmark_count * 3.0 + share_count * 5.0 + feedback_score * 2.0
    )
...
async def get_personalized_news(
    request: Request, limit: int = 20, current_user: TokenData = Depends(get_current_user)
):
    """
    개인화된 뉴스 피드
    ...
    try:
        ...
        # 개인화 에이전트 호출
        # personalization_agent = PersonalizationAgent()  # TODO: Fix import
        ...
        try:
            if personalization_agent:
                ...
                personalized_result = {
                    "articles": [{"title": "Mock article", "content": "Mock content"}],
                    "overall_score": 0.8,
                }
