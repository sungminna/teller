...
class RedisCacheManager:
    ...
    async def cache_voice_file_metadata(self, file_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            return await client.set(voice_key, json.dumps(voice_payload, default=str))
        except Exception as e:
    ...
    async def update_realtime_stats(
        self, stat_type: str, value: Union[int, float], labels: Optional[Dict[str, str]] = None
    ) -> bool:
    ...
    async def get_realtime_stats(
        self, stat_type: str, labels: Optional[Dict[str, str]] = None
...
class RedisTimeSeries:
    ...
    async def create_timeseries(
        self, key: str, retention_ms: int = 86400000, labels: Optional[Dict[str, str]] = None
    ) -> bool:
    ...
    async def add_sample(
        self, key: str, timestamp: Optional[int] = None, value: float = 1.0
    ) -> bool:
...
async def create_redis_client() -> redis.Redis:
    ...
    try:
        ...
        logger.info(
            f"✅ Enhanced Redis client created: {settings.REDIS_HOST}:{settings.REDIS_PORT}"
        )
        return client
...
async def cache_set(
    key: str, value: Any, expire: Optional[int] = None, serialize: bool = True
) -> bool:
...
async def cache_get(key: str, deserialize: bool = True, default: Any = None) -> Any:
...
async def cache_lrange(
    key: str, start: int = 0, end: int = -1, deserialize: bool = True
) -> List[Any]:
    """Get range from list"""
    try:
        client = await get_redis_client()
