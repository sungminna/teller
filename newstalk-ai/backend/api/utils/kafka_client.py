...
async def create_kafka_producer() -> AIOKafkaProducer:
    ...
    try:
        ...
        logger.info(f"✅ Enhanced Kafka producer created: {settings.KAFKA_BOOTSTRAP_SERVERS}")
        return producer
...
async def get_kafka_admin() -> AIOKafkaAdminClient:
    ...
    if _kafka_admin is None:
        settings = get_settings()
        _kafka_admin = AIOKafkaAdminClient(
            bootstrap_servers=settings.KAFKA_BOOTSTRAP_SERVERS
        )
    ...
    try:
        consumer = AIOKafkaConsumer(
            *topics,
            ...
            fetch_max_wait_ms=settings.KAFKA_CONSUMER_FETCH_MAX_WAIT_MS,
            fetch_min_bytes=1,
        ...
        logger.info(f"✅ Enhanced Kafka consumer created for topics: {topics}")
        return consumer
...
async def publish_raw_news(news_data: Dict[str, Any]) -> bool:
    ...
    return await send_message(
        KAFKA_TOPICS['raw_news'],
        news_data,
        key=f"news_{news_data.get('id', uuid.uuid4())}"
    )
