...
class LangfuseManager:
    ...
    async def track_pipeline_execution(
        ...
        try:
            ...
            await self._store_pipeline_metrics(pipeline_name, total_time, success, articles_processed)
            
            logger.info(f"Pipeline execution tracked - {pipeline_name}: {total_time:.2f}s, {articles_processed} articles")
    ...
    async def manage_prompt_versions(
        ...
        try:
            ...
            await cache_manager.cache_set(cache_key, {
    ...
    async def _send_cost_alert(self, total_cost: float):
        ...
        try:
            ...
            await cache_manager.cache_set(
                f"cost_alert:{datetime.utcnow().timestamp()}",
                alert_data,
                expire=3600
            )
    ...
    async def _store_performance_metrics(self, model: str, usage: Dict[str, Any], cost: float):
        ...
        try:
            ...
            new_stats = {
                ...
                "total_tokens": int(current_stats.get("total_tokens", 0)) + usage.get("total_tokens", 0),
    ...
    async def _store_agent_metrics(self, agent_name: str, execution_time: float, success: bool):
        ...
        try:
            ...
            await timeseries.add_sample(f"agent_success_rate:{agent_name}", value=1.0 if success else 0.0)
    ...
    async def _store_pipeline_metrics(self, pipeline_name: str, total_time: float, success: bool, articles_processed: int):
        """Store pipeline performance metrics"""
        try:
            from .redis_client import timeseries
            
            # Update TimeSeries
            await timeseries.add_sample(f"pipeline_execution_time:{pipeline_name}", value=total_time)
            await timeseries.add_sample(f"pipeline_articles_processed:{pipeline_name}", value=articles_processed)
            await timeseries.add_sample(f"pipeline_success_rate:{pipeline_name}", value=1.0 if success else 0.0)
    ...
    async def _calculate_performance_insights(self, time_range_hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if insights["model_performance"]:
                slowest_model = max(insights["model_performance"].items(), 
            ...
            for key in model_keys:
                ...
                if stats:
                    model_insights[model_name] = {
                        ...
                        "avg_cost_per_call": float(stats.get("total_cost", 0)) / max(int(stats.get("total_calls", 1)), 1),
    ...
    async def _get_cost_optimization_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "optimization_opportunities": await self._identify_optimization_opportunities(model_costs)
            }
    ...
    async def _calculate_potential_savings(self, model_costs: Dict[str, float]) -> float:
        ...
        expensive_models_cost = sum(cost for model, cost in model_costs.items() 
    ...
    async def _identify_optimization_opportunities(self, model_costs: Dict[str, float]) -> List[str]:
        ...
        for model, cost in model_costs.items():
            if cost > 5.0:  # More than $5 in 7 days
                if "gpt-4" in model.lower():
                    opportunities.append(f"Consider using GPT-3.5-turbo for simple tasks instead of {model}")
                elif "claude-3-opus" in model.lower():
                    opportunities.append(f"Consider using Claude-3-Sonnet for most tasks instead of {model}")
        
        # Check for model diversity
        if len(model_costs) == 1:
            opportunities.append("Consider using multiple models for different task types to optimize costs")
