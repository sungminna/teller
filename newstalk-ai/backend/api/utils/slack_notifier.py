...
class SlackNotifier:
    ...
    async def send_alert(
        ...
        try:
            ...
            if success:
                ...
                await self._store_alert_analytics(message, severity, category, metadata)
    ...
    async def send_business_alert(
        self, metric_name: str, current_value: float, expected_value: float, impact: str = None
    ):
        ...
        return await self.send_alert(
            message=message, severity=severity, category=AlertCategory.BUSINESS, metadata=metadata
        )
    ...
    async def _format_alert(
        ...
        if playbook and playbook in self.playbooks:
            ...
            # Add response steps
            steps_text = "\n".join(
                [f"{i+1}. {step}" for i, step in enumerate(playbook_info["steps"])]
            )
            attachment["fields"].append(
                {"title": "🔧 Response Steps", "value": f"```{steps_text}```", "short": False}
            )
            ...
            if playbook_info.get("contacts"):
                ...
                attachment["fields"].append(
                    {"title": "👥 Contacts", "value": contacts_text, "short": False}
                )
    ...
    async def _is_rate_limited(self, message: str, category: AlertCategory) -> bool:
        ...
        try:
            ...
            if (
                last_category_alert
                and (current_time - float(last_category_alert)) < self.rate_limits["category_limit"]
            ):
                return True
            ...
            if (
                last_global_alert
                and (current_time - float(last_global_alert)) < self.rate_limits["global_limit"]
            ):
                return True
    ...
    async def _update_rate_limit_cache(self, message: str, category: AlertCategory):
        ...
        try:
            ...
            await cache_manager.cache_set(
                "alert_rate_limit:global", current_time, expire=self.rate_limits["global_limit"]
            )
    ...
    async def _collect_daily_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for severity in AlertSeverity:
                count = await cache_manager.cache_get(
                    f"daily_alerts:{today}:{severity.value}", default=0
                )
            ...
            for category in AlertCategory:
                count = await cache_manager.cache_get(
                    f"daily_alerts:{today}:{category.value}", default=0
                )
            ...
            metrics = {
                ...
                "articles_processed": await cache_manager.cache_get(
                    "metrics:daily_articles", default=0
                ),
                "user_satisfaction": await cache_manager.cache_get(
                    "metrics:user_satisfaction", default=0.0
                ),
...
async def check_thresholds_and_alert():
    ...
    try:
        ...
        avg_response_time = await cache_manager.cache_get(
            "metrics:api_avg_response_time", default=0.0
        )
        ...
        if float(daily_cost) > slack_notifier.thresholds["daily_cost"]:
            await slack_notifier.send_cost_alert(
                float(daily_cost), slack_notifier.thresholds["daily_cost"]
            )
