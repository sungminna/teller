...
from .routes import (
    health,      # 시스템 헬스체크 및 모니터링
    news,        # 뉴스 CRUD, 트렌딩, 개인화 피드
    users,       # 사용자 관리, 인증, 프로필
    ai,          # AI 기반 Q&A, 음성합성, 분석
    streaming    # 실시간 스트리밍, SSE, WebSocket
)
...
# 🔧 유틸리티 임포트 - 외부 시스템 연동
from .utils.kafka_client import stream_processor, close_all_kafka_connections  # Kafka 스트리밍
from .utils.redis_client import close_redis_client                             # Redis 캐싱
from ..shared.config.settings import get_settings                              # 설정 관리
    ...
    try:
        ...
        return {
            ...
            "system_status": {
                "redis": redis_status,
                "kafka": kafka_status,
                "streaming": "enabled" if settings.ENABLE_STREAMING else "disabled"
            },
            "endpoints": {
                "api_docs": "/docs",
                "health_check": "/health",
...
async def get_system_info():
    ...
    try:
        ...
        return {
            ...
            "kafka_topics": [
                settings.KAFKA_RAW_NEWS_TOPIC,
                settings.KAFKA_PROCESSED_NEWS_TOPIC,
                settings.KAFKA_USER_FEEDBACK_TOPIC,
