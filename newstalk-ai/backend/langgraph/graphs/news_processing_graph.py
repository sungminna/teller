...
class NewsProcessingGraph:
    ...
    def __init__(self):
        ...
        logger.info(
            "🎯 NewsTalk AI Processing Graph initialized with 3 specialized agents (Stage 3)"
        )
    ...
    def _build_graph(self) -> Pregel:
        ...
        workflow.add_conditional_edges(
            ...
            {"approved": "completion", "retry": "news_analysis", "end": END},
        )
    ...
    async def _personalization_node(self, state: NewsState) -> NewsState:
        ...
        try:
            logger.info(
                f"👤 Starting personalization + storytelling for article {state.article_id}"
            )
    ...
    def _should_proceed_after_analysis(
        self, state: NewsState
    ) -> Literal["continue", "quality_check", "human_review", "end"]:
        ...
        if hasattr(state, "fact_check_result") and state.fact_check_result:
            if state.fact_check_result.credibility_score < 0.95:
                logger.warning(
                    f"Low credibility score: {state.fact_check_result.credibility_score}"
                )
                return "human_review"
    ...
    def _should_proceed_after_personalization(
        self, state: NewsState
    ) -> Literal["continue", "quality_check", "human_review", "end"]:
        ...
        if hasattr(state.personalization_result, "personalization_score") and hasattr(
            state.personalization_result.personalization_score, "overall_score"
        ):
            if state.personalization_result.personalization_score.overall_score < 4.5:
                logger.warning(
                    f"Low personalization score: {state.personalization_result.personalization_score.overall_score}"
                )
                return "quality_check"

        return "continue"

    def _should_proceed_after_voice_synthesis(
        self, state: NewsState
    ) -> Literal["continue", "quality_check", "human_review", "end"]:
        """음성 합성 후 다음 단계 결정"""
        if state.errors:
    ...
    def _route_after_quality_check(
        ...
        "retry_analysis", "retry_personalization", "retry_voice", "human_review", "continue", "end"
    ]:
        """🎯 품질 검증 후 라우팅 (3개 에이전트)"""
        ...
        # 🎯 3개 전문 에이전트 재실행 분기
        if (
            "Low news analysis score" in validation_issues
            or "Fact check accuracy below 95%" in validation_issues
        ):
            return "retry_analysis"
        elif (
            "Low personalization score" in validation_issues
            or "Low storytelling engagement" in validation_issues
        ):
    ...
    async def process_news(
        ...
        try:
            ...
            logger.info(
                f"NewsTalk AI processing completed for article {article_id} - Stage: {final_state.processing_stage}"
            )
            return final_state
