...
from ...shared.models.news import UserProfile, UserInteraction, PreferenceScore
...
class PreferenceLearner:
    ...
    async def predict_satisfaction(self, user_id: str, content_features: Dict) -> float:
        ...
        try:
            ...
            if self.model_metadata['last_updated'] is None:
                # 기본 휴리스틱 사용
                return await self._heuristic_satisfaction_prediction(
                    user_profile, content_features
                )
    ...
    async def optimize_content_selection(self, user_id: str, candidate_contents: List[Dict]) -> List[Dict]:
        """
    ...
    async def get_personalization_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            insights = {
                ...
                'satisfaction_gap': self.config.satisfaction_target - user_profile.satisfaction_score,
                'preference_analysis': preference_analysis,
                'satisfaction_trend': satisfaction_trend,
                'improvement_suggestions': improvement_suggestions,
                'personalization_strength': await self._calculate_personalization_strength(user_profile),
    ...
    def _calculate_interaction_satisfaction(self, interaction: UserInteraction) -> float:
        ...
        base_score = satisfaction_mapping.get(
            InteractionType(interaction.interaction_type), 3.0
        )
    ...
    def _calculate_interaction_engagement(self, interaction: UserInteraction) -> float:
        ...
        base_score = engagement_mapping.get(
            InteractionType(interaction.interaction_type), 0.3
        )
    ...
    async def _update_models(self):
        ...
        try:
            ...
            X_train, X_test, y_sat_train, y_sat_test, y_eng_train, y_eng_test = train_test_split(
                features, satisfaction_targets, engagement_targets, 
    ...
    def _calculate_feature_importance(self) -> Dict[str, float]:
        """특성 중요도 계산"""
        feature_names = [
            'category', 'keyword_count', 'sentiment', 'complexity',
            'credibility', 'time_of_day', 'day_of_week', 'device_type',
            'session_duration'
        ]
    ...
    async def _heuristic_satisfaction_prediction(self, user_profile: UserProfile, content_features: Dict) -> float:
    ...
    async def _heuristic_engagement_prediction(self, user_profile: UserProfile, content_features: Dict) -> float:
        """휴리스틱 기반 참여도 예측"""
        base_score = 0.5
    ...
    async def _create_prediction_features(self, user_profile: UserProfile, content_features: Dict) -> List[float]:
        """예측을 위한 특성 벡터 생성"""
        return [
            ...
            0.5,  # 기본 디바이스 타입
            min(user_profile.average_session_duration / 3600.0, 1.0) if user_profile.average_session_duration else 0.5,
        ]
    ...
    async def _calculate_real_time_preferences(self, user_id: str, learning_feature: LearningFeature) -> Dict[str, float]:
        """실시간 선호도 점수 계산"""
        return {
    ...
    async def _generate_improvement_suggestions(self, user_profile: UserProfile, preference_analysis: Dict, satisfaction_trend: Dict) -> List[str]:
        """개선 제안 생성"""
        suggestions = []
