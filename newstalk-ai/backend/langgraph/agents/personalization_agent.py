...
class AdvancedPersonalizationAgent:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.tfidf_vectorizer = TfidfVectorizer(
                max_features=1000, stop_words=None, ngram_range=(1, 2)  # 한국어 불용어 리스트 필요
            )
    ...
    def _initialize_monitoring(self):
        ...
        try:
            # Langfuse 추적
            if (
                self.settings.langgraph.langfuse_public_key
                and self.settings.langgraph.langfuse_secret_key
            ):
    ...
    async def personalize_content(self, state: NewsState, user_id: str) -> NewsState:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                logger.info(
                    f"Starting content personalization for user {user_id}, article {state.article_id}"
                )
    ...
    async def _get_or_create_user_profile(self, user_id: str, trace) -> UserProfile:
        ...
        try:
            ...
            if span:
                span.update(
                    output={"profile_created": True, "interactions": profile.total_interactions}
                )
    ...
    def _should_update_profile(self, profile: UserProfile) -> bool:
        ...
        try:
            ...
            return (
                ...
                or len(self.user_interactions.get(profile.user_id, []))
    ...
    def _update_category_preferences(
        self, profile: UserProfile, interactions: List[UserInteraction]
    ) -> UserProfile:
        """카테고리 선호도 업데이트"""
        try:
    ...
    def _update_keyword_preferences(
        self, profile: UserProfile, interactions: List[UserInteraction]
    ) -> UserProfile:
        """키워드 선호도 업데이트"""
        try:
            ...
            # 상위 키워드만 유지 (메모리 효율성)
            top_keywords = dict(
                sorted(keyword_scores.items(), key=lambda x: x[1], reverse=True)[:100]
            )
        ...
        try:
            ...
            if total_interactions > 0:
                for hour, count in hour_counts.items():
                    ...
                    profile.time_preferences[hour_str] = 0.3 * preference + 0.7 * current_pref

            # 활성 시간대 업데이트
            if hour_counts:
                profile.active_hours = sorted(
                    hour_counts.keys(), key=hour_counts.get, reverse=True
                )[:6]
    ...
    def _update_behavior_patterns(
        ...
        try:
            ...
            # 선호 콘텐츠 길이 추정
            content_lengths = [
                interaction.context.get("content_length", 0) for interaction in interactions
            ]
    ...
    async def _calculate_personalization_score(
        self, state: NewsState, profile: UserProfile, trace
    ) -> float:
        """개인화 점수 계산"""
        ...
        try:
            ...
            # 4. 콘텐츠 길이 선호도
            content_length = len(state.content)
            length_score = self._calculate_length_preference_score(
                content_length, profile.preferred_length
            )
    ...
    def _calculate_length_preference_score(
        self, content_length: int, preferred_length: str
    ) -> float:
        """콘텐츠 길이 선호도 점수 계산"""
        try:
    ...
    async def _generate_recommendation_explanation(
        self, state: NewsState, profile: UserProfile, score: float, trace
        ...
        try:
            ...
            if hasattr(state, "trend_analysis_result") and state.trend_analysis_result:
                keywords = state.trend_analysis_result.keywords
                matching_keywords = [
                    kw for kw in keywords if profile.keyword_preferences.get(kw, 0.0) > 0.5
                ]
