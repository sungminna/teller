...
from ..state.news_state import NewsState, ProcessingStage, TrendAnalysisResult
    ...
    async def analyze_comprehensive(self, state: NewsState) -> NewsState:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                # 결과 처리
                keywords, sentiment_score, trending_score, virality_potential, trend_category = (
                    results[:5]
                )
                ...
                analysis_result = TrendAnalysisResult(
                    trending_score=(
                        trending_score if not isinstance(trending_score, Exception) else 0.0
                    ),
                    trend_category=(
                        trend_category.value
                        if not isinstance(trend_category, Exception)
                        else TrendCategory.BREAKING_NEWS.value
                    ),
                    keywords=keywords if not isinstance(keywords, Exception) else [],
                    sentiment_score=(
                        sentiment_score if not isinstance(sentiment_score, Exception) else 0.0
                    ),
                    virality_potential=(
                        virality_potential if not isinstance(virality_potential, Exception) else 0.0
                    ),
...
JSON 형식으로 반환:
{
    ...
    def _clean_and_deduplicate_keywords(self, keywords: List[str]) -> List[str]:
        ...
        for keyword in keywords:
            ...
            if (
                len(clean_keyword) >= 2
                and len(clean_keyword) <= 20
                and clean_keyword.lower() not in seen
            ):
    ...
    async def _extract_spacy_keywords_async(self, text: str) -> List[str]:
        ...
        try:
            ...
            keywords = await loop.run_in_executor(
                self.executor, self._extract_spacy_keywords_sync, text
            )
...
JSON 형식으로 반환:
{
            ...
            if self.redis_client:
                try:
                    await self.redis_client.setex(
                        key, self.config.cache_ttl_hours * 3600, serialized_data
                    )
                except Exception as e:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(self.metrics.confidence_scores.values())
            / max(1, len(self.metrics.confidence_scores)),
        }
