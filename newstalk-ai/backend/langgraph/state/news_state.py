...
class FactCheckResult:
    ...
    fact_check_status: str            # 검증 상태: "verified"(검증됨) | "disputed"(논란) | "false"(거짓) | "mixed"(혼재)
...
class NewsState:
    ...
    def update_stage(self, stage: ProcessingStage):
        ...
        - 메트릭 수집 트리거
        """
        previous_stage = self.processing_stage
    ...
    def is_processing_complete(self) -> bool:
        ...
        """
        return (
            self.processing_stage == ProcessingStage.COMPLETED and
            self.error is None and
            self.trend_analysis_result is not None and
            self.personalization_result is not None and
            self.voice_synthesis_result is not None
        )
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_metrics": {
                "credibility_score": self.fact_check_result.credibility_score if self.fact_check_result else None,
                "personalization_score": self.personalization_result.relevance_score if self.personalization_result else None,
                "synthesis_quality": self.voice_synthesis_result.synthesis_quality if self.voice_synthesis_result else None,
            },
            "error": self.error,
    ...
    def requires_human_review(self) -> bool:
        ...
        # 논란 상태인 경우
        if self.fact_check_result and self.fact_check_result.fact_check_status in ["disputed", "false"]:
            return True
