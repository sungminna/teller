...
class DataValidatorOperator(BaseOperator):
    ...
    def _get_raw_articles(self) -> List[NewsArticle]:
        ...
        try:
            return (
                ...
                .filter(NewsArticle.status == ProcessingStatus.RAW)
    ...
    def _validate_articles(self, articles: List[NewsArticle]) -> Dict[str, Any]:
        ...
        results = {"processed": 0, "valid": 0, "invalid": 0, "duplicates": 0}
        ...
        try:
            for article in articles:
                ...
                if (
                    ...
                    and validation["quality_score"] >= self.quality_threshold
                ):
...
class DataQualityReportOperator(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, report_hours: int = 24, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def execute(self, context):
        ...
        try:
            ...
            articles = (
                session.query(NewsArticle).filter(NewsArticle.collected_at >= cutoff_time).all()
            )
    ...
    def _generate_quality_report(self, articles: List[NewsArticle]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "validation_success_rate": (
                (valid_articles / total_articles * 100) if total_articles > 0 else 0.0
            ),
