...
class LangGraphTriggerOperator(BaseOperator):
    ...
    def _get_published_articles(self) -> List[NewsArticle]:
        ...
        try:
            return session.query(NewsArticle).filter(
                NewsArticle.status == ProcessingStatus.PUBLISHED
    ...
    async def _trigger_single_workflow(self, client: httpx.AsyncClient, article: NewsArticle) -> Dict[str, Any]:
        ...
        with httpx.Client(timeout=30) as client:
            for article in articles:
                try:
                    ...
                    response = client.get(
                        f"{self.langgraph_endpoint}/status/{workflow_id}"
                    )
