...
from shared.models.news import NewsSource, NewsArticle, ProcessingLog, ProcessingStatus
...
class RSSCollectorOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        logger.info(f"Processing rate: {total_articles / (processing_time / 3600):.0f} articles/hour")
        ...
        return {
            ...
            'articles_per_source': {source.id: len(articles) for source, articles in collected_articles.items()},
    ...
    def _collect_feeds_concurrent(self, sources: List[NewsSource]) -> Dict[NewsSource, List[Dict[str, Any]]]:
        ...
        with ThreadPoolExecutor(max_workers=self.concurrent_feeds) as executor:
            ...
            future_to_source = {
                executor.submit(self._collect_single_feed, source): source 
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if feed_data.bozo and hasattr(feed_data, 'bozo_exception'):
                    logger.warning(f"RSS parsing warning for {source.name}: {feed_data.bozo_exception}")
                ...
                for entry in feed_data.entries[:self.max_articles_per_source]:
                    ...
                    if article_data:
                        ...
                        if validation_result['is_valid'] and validation_result['quality_score'] >= self.quality_threshold:
    ...
    def _extract_article_data(self, entry: Dict[str, Any], source: NewsSource) -> Optional[Dict[str, Any]]:
...
class AsyncRSSCollectorOperator(BaseOperator):
    ...
    async def _async_collect(self):
        ...
        async with aiohttp.ClientSession(
            ...
            tasks = [
                self._collect_feed_async(session, source, semaphore) 
                for source in sources
            ]
        ...
        processing_time = time.time() - start_time
        logger.info(f"Async collection completed: {total_articles} articles in {processing_time:.2f}s")
    ...
    async def _collect_feed_async(self, session: aiohttp.ClientSession, source: NewsSource, semaphore: asyncio.Semaphore):
