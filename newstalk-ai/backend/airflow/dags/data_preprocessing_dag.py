...
def clean_and_normalize_text(**context):
    ...
    try:
        ...
        articles = (
            ...
            .filter(NewsArticle.status == ProcessingStatus.AI_ANALYZED)
            .limit(100)
            .all()
        )
...
def generate_embeddings(**context):
    ...
    try:
        articles = (
            ...
            .limit(50)
            .all()
        )  # Smaller batch for embedding generation
                    ...
                    if i != j and other_article.embedding_vector:
                        ...
                        if similarity > 0.7:  # Only store high similarity
                            similarity_scores.append(
                                {"article_id": other_article.id, "similarity": float(similarity)}
                            )
...
text_cleaning = PythonOperator(
    task_id="clean_normalize_text", python_callable=clean_and_normalize_text, dag=dag
)

metadata_extraction = PythonOperator(
    task_id="extract_metadata", python_callable=extract_metadata, dag=dag
)

embedding_generation = PythonOperator(
    task_id="generate_embeddings", python_callable=generate_embeddings, dag=dag
)

similarity_calculation = PythonOperator(
    task_id="calculate_similarity", python_callable=calculate_similarity_scores, dag=dag
)

preprocessing_finalization = PythonOperator(
    task_id="finalize_preprocessing", python_callable=finalize_preprocessing, dag=dag
)
...
(
    text_cleaning
    >> metadata_extraction
    >> [embedding_generation, similarity_calculation]
    >> preprocessing_finalization
    >> report_task
    >> health_check
)
