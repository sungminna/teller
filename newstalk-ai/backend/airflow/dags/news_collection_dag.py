...
def collect_and_stream_news(**context):
    ...
    # Note: These imports would be available in actual deployment
    ...
    streaming_results = {"successful": len(articles), "failed": 0}
...
async def stream_articles_to_kafka(articles):
    ...
    logger.info(
        f"🚀 Starting optimized Kafka streaming: {total_articles} articles, "
    ...
    logger.info(
        f"🎯 Kafka streaming completed: {results['successful']}/{total_articles} successful "
        ...
        async for news_data in consume_news_stream():
            ...
            try:
                ...
                await publish_realtime_update(
                    ...
                    {"article_id": news_data.get("id"), "title": news_data.get("title")},
                )
                ...
                if result.get("success"):
                    ...
                    await publish_realtime_update(
                        ...
                        {"article_id": news_data.get("id"), "error": result.get("error")},
                    )

            except Exception as e:
                processing_results["failed"] += 1
                await publish_realtime_update(
                    "ai_processing_error", {"article_id": news_data.get("id"), "error": str(e)}
                )
...
def generate_realtime_metrics(**context):
    ...
    async def publish_metrics():
        ...
        validation_result = context["task_instance"].xcom_pull(
            task_ids="validate_and_process_stream"
        )
        ...
        metrics = {
            ...
            "collection": {
                ...
                "streaming_success_rate": (
                    collection_result.get("streamed_articles", 0)
                    / max(collection_result.get("total_articles", 1), 1)
                )
                * 100,
            },
            "validation": {
                ...
                "validation_rate": (
                    validation_result.get("valid", 0)
                    / max(validation_result.get("processed", 1), 1)
                )
                * 100,
            },
            "ai_processing": {
                ...
                "success_rate": (
                    ai_result.get("successful", 0) / max(ai_result.get("triggered", 1), 1)
                )
                * 100,
            },
            "performance": {
                "pipeline_under_5min": pipeline_duration < 300,
                "articles_per_second": collection_result.get("total_articles", 0)
                / max(pipeline_duration, 1),
                "processing_efficiency": (
                    ai_result.get("successful", 0)
                    / max(collection_result.get("total_articles", 1), 1)
                )
                * 100,
            },
        ...
        await timeseries.add_sample(
            "articles_processed", value=collection_result.get("total_articles", 0)
        )

        # Update real-time stats in Redis
        await cache_manager.update_realtime_stats("pipeline_performance", pipeline_duration)
        await cache_manager.update_realtime_stats(
            "article_throughput", collection_result.get("total_articles", 0)
        )
...
def monitor_pipeline_performance(**context):
    ...
    async def check_performance():
        ...
        duration_samples = await timeseries.get_range(
...
def comprehensive_health_check(**context):
    ...
    from api.utils.kafka_client import get_kafka_admin, get_kafka_producer
...
# Define validate_stream task
validate_stream = PythonOperator(
    task_id="validate_and_process_stream",
    python_callable=validate_and_process_stream,
    provide_context=True,
    execution_timeout=timedelta(minutes=2),
    dag=dag,
)
